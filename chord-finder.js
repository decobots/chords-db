//https://codepen.io/Mia-Pritchina/pen/OJeJRzJ
...
class Instrument {
  ...
  string_fingerings(chord, options, fingering, res, i) {
    ...
    if (i == this.strings.length) {
      ...
      if (
        notes.size == chord.notes.length &&
        (!force_root || sounding[0].color === 0)
      )
        res.push(fingering);
      return;
    }
    ...
    if (!bar) {
      this.string_fingerings(
        chord,
        options,
        fingering.push(new Finger(i, 0, null, null)),
        res,
        i + 1
      );
    }
        ...
        if (fingering.max_reach(fret) < max_reach) {
          if (!bar && fret > 0 && i != this.strings.length - 1) {
            ...
            this.string_fingerings(
              ...
              options,
              fingering.push(
                new Finger(i, fret, n, NOTES[chord.notes[n]], true)
              ),
              res,
              i + 1
            );
          ...
          this.string_fingerings(
            chord,
            options,
            fingering.push(new Finger(i, fret, n, NOTES[chord.notes[n]])),
            res,
            i + 1
          );
        }
  ...
  chord_fingerings(chord, options) {
    ...
    filtered = filtered.filter(
      (fs) => !filtered.some((other) => fs != other && fs.isSubSetOf(other))
    );
...
class Fingering {
  constructor(fingers) {
    ...
    //======================================================================= Original code
    /* this.signature = this.fingers
      .map((f) => (f.mute ? "x" : f.fret + (f.bar ? "b" : "")))
      .join("|");
      */

    //======================================================================= My version
    this.signature = this.fingers
      .map((f) => (f.mute ? "x" : f.fret + (f.bar ? "" : "")))
      .join("");
  }
  ...
  get no_fingers() {
    return this.sounding.filter(
      (f) => (!this.bar || (this.bar.fret != f.fret && this.bar != f)) && f.fret
    ).length;
  }
  ...
  lefty(instrument) {
    return new Fingering(
      this.fingers.map(
        (f) =>
          new Finger(
            instrument.strings.length - 1 - f.string,
            f.fret,
            f.color,
            f.label,
            f.bar ? "left" : false
  ...
  get pointless_bar() {
    const bar = this.bar;
    if (!bar) return false;
    return !this.fingers.some(
      (f) => f.string > bar.string && f.fret == bar.fret
    );
  }
  ...
  isSubSetOf(other) {
    const res = this.sounding.every((f) =>
      other.sounding.some((o) => f.string == o.string && f.fret == o.fret)
    );
    ...
    if (
      ...
      this.bar &&
      other.bar &&
      this.bar.fret == other.bar.fret &&
      this.bar.string < other.bar.string
    )
      return false;
    return res;
  }
...
const TRIADS_LOOKUP = Object.fromEntries(
  TRIADS.map((t) => [t.notes, t.symbol])
);
...
class Chord {
  ...
  get type() {
    ...
    return (
      (fingerprint.length > 3 ? SEVENS_LOOKUP[fingerprint[3]] : "") +
      TRIADS_LOOKUP[fingerprint.slice(0, 3)]
    );
  }
...
class Scale {
  ...
  chord(degree) {
    return new Chord(
      [0, 2, 4].map((d) => this.notes[(degree + d) % this.notes.length])
    );
  }
}
...
const INSTRUMENTS = Object.fromEntries(
  [
    ...
    new Instrument("UkuleleBaritone", [2, 7, 11, 4]), // DGBE
    new Instrument("Banjo", [2, 7, 11]), // DGBD
    new Instrument("Banjo Open C", [0, 7, 0, 4]) // CGCE
  ].map((i) => [i.name, i])
);

function format_to_notes(array_of_numbers) {
  var res = [];
  for (var i = 0; i < array_of_numbers.length; i++) {
    res.push(NOTES[array_of_numbers[i]]);
  }
  return res;
}
